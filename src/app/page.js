...
import { trackVslPlay } from '@/lib/funnel-events';
...
export default function Landing() {
  ...
  /* GA4 `video_play` is fired here rather than inside VslFrame, because both
     controls have to converge on one place or the event double-counts — and
     because VslFrame is a generic frame: firing from inside it would make any
     future reuse (a testimonial, a thank-you clip) silently report as a hero
     VSL play. The state flip is what mounts the autoplay iframe, so this is
     the play. */
  const beginVsl = () => {
    trackVslPlay();
    setVslPlaying(true);
    ...
    <main className="sdp-root">
      ...
      <section className="sdp-hero" id="hero">
        <div className="sdp-wrap pa-hero-inner">
          ...
          <div ref={vslRef}>
            <VslFrame playing={vslPlaying} onPlay={beginVsl} />
          </div>
