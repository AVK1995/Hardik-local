...
import { trackCallBooked } from '@/lib/funnel-events';
...
export default function CalendlyEmbed({ id = 'pick-slot' }) {
  ...
  useEffect(() => {
    ...
    const onMessage = (e) => {
      ...
      /* GA4 `book_call`. There is no button of ours to attach to — the whole
         booking happens inside Calendly's iframe — so this origin-checked
         postMessage is the only place a real booking is observable. It fires
         on the confirmed booking, not on the embed opening, which is the
         signal actually worth counting. */
      trackCallBooked();
