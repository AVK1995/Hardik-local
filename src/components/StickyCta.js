...
import { cta, sticky } from '@/lib/content';
  ...
  return (
    <div className={`sdp-stuck${on ? ' on' : ''}`} ref={barRef} aria-hidden={!on}>
      <div className="sdp-wrap sdp-stuck-inner">
        ...
        {/* The sticky bar is a separate component from CtaLockup and is the
            CTA that is easiest to forget when inventorying them. It fires the
            same once-per-browser add-to-cart. */}
        <a
          className="sdp-cta"
          href="/checkout"
          tabIndex={on ? 0 : -1}
          onClick={trackCheckoutCtaClick}
        >
